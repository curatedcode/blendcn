...
const darkColors = Object.fromEntries(
	radixScaleNames.map((scaleName) => [
		scaleName,
		Object.values(radixScaleColors[`${scaleName}Dark`]).map((str) =>
			new Color(str).to("oklch"),
		),
	]),
) as Record<(typeof radixScaleNames)[number], ArrayOf12<Color>>;
...
const lightGrayColors = Object.fromEntries(
	radixGrayScaleNames.map((scaleName) => [
		scaleName,
		Object.values(radixScaleColors[scaleName]).map((str) =>
			new Color(str).to("oklch"),
		),
...
function getScaleFromColor(
	...
	const grayScaleNamesStr = radixGrayScaleNames as readonly string[];
	...
	if (!colorA || !colorB) {
		throw new Error("Unable to find closest colors");
	}
	...
	if (!scaleA || !scaleB) {
		throw new Error("Unable to find base scale");
	}
	...
	if (!baseColor) {
		throw new Error("Unable to get base closest color");
	}
	...
	if (ratioL > 1) {
		...
		for (let i = 0; i < ease.length; i++) {
			const thisEase = ease[i];
			if (!thisEase) continue;

			const metaRatio = (ratioL - 1) * (maxRatio / (maxRatio - 1));
			ease[i] = ratioL > maxRatio ? 0 : Math.max(0, thisEase * (1 - metaRatio));
		}
	}
...
function _transposeProgressionEnd(
	to: number,
	arr: number[],
	curve: [number, number, number, number],
) {
